...
-- 10
SELECT top(1) CONCAT('Les ' , i.FIRST_BOOTS_BUILT,  ' sont les bottes les plus faites avec ', COUNT(i.FIRST_BOOTS_BUILT), ' bottes faites') AS "Les bottes les plus faites"
FROM ITEMS i
GROUP BY i.FIRST_BOOTS_BUILT
ORDER BY  COUNT(i.FIRST_BOOTS_BUILT) DESC


-- 11
SELECT top(1) CONCAT(i.FIRST_ITEM_BUILT,  ' est l"item le plus fait avec ', COUNT(i.FIRST_ITEM_BUILT), ' faits') AS "L'item le plus fait"
FROM ITEMS i
GROUP BY i.FIRST_ITEM_BUILT
ORDER BY COUNT(i.FIRST_ITEM_BUILT) DESC


-- 12 
SELECT top(1) CONCAT(i.FIRST_ITEM_BUILT,  ' est l"item le plus fait avec ', COUNT(i.FIRST_ITEM_BUILT), ' faits') AS "L'item le plus fait sur les CAC"
FROM ITEMS i
INNER JOIN CHAMPSTATS cs
ON(i.CHAMP_ID = cs.CHAMP_ID)
WHERE cs.CAC LIKE 1
GROUP BY i.FIRST_ITEM_BUILT
ORDER BY COUNT(i.FIRST_ITEM_BUILT) DESC
